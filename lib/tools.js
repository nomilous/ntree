...
// TODO: expensive in big tree, make plan
Tools.prototype.getNestedVerticesWithOnlyThisSource = function(vertex, source) {
  ...
  var recurse = function(vertex) {
    if (vertex.sources.length === 1) {
      ...
      if (vertex.sources.filter(function(s) {
        return s.filePath === source.filePath
      }).length > 0) {
        // got this source plus others in vertex, recurse further
        Object.keys(vertex.vref).forEach(function(key) {
          if (key === '__') return;
          recurse(vertex.vref[key].__);
        });
        vertices.others.push(vertex);
      }
    }
    // Object.keys(vertex.vref).forEach(function(key) {
    //   if (key === '__') return;
