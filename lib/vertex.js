...
Vertex.prototype.loadDirectory = Promise.promisify(function(callback) {
  ...
  .then(function(nextList) {
    ...
    var lazy = _this._tree._meta.lazy;

    if (lazy) return callback(null, _this);

    // nextList (directories) recurse if not lazy
