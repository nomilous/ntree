...
Vertex.prototype.loadDirectory = Promise.promisify(function(callback) {
  ...
  .then(function(nextList) {
    ...
    debugs.loadDirectory('(%s) resolved with next list', name);
  ...
  .then(function() {
    ...
    debugs.loadDirectory('(%s) calling back', name);
...
Vertex.prototype.loadFile = function() {
  ...
  try {
    ...
    this.attachContent(serializer.decode(this));
 ...
 * @api private
 * @param {String} key
 * @param {Agent|Edge} agent that detected the change
 *
 */

Vertex.prototype.created = function(key, agent) {

  var name = this._info.name;
  var type = this._info.stat.isDirectory() ? 'directory' : 'file';

  debugs.created(
    '(%s) handling new key: \'%s\', at path: \'%s\', into: \'%s\'',
    name,
    key,
    agent.routeString,
    type
