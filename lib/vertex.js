...
Vertex.prototype.unloadSource = function(source) {
  ...
  var deletedTref, deletedVref;
  var thiskey = source.route.last;

  // full delete if source is the only source for this vertex
  if (this.sources.length == 1) {
    deletedTref = this.parent.value[thiskey];
    deletedVref = this.parent.vref[thiskey];
    deletedVref.__.deleted = true;

    delete this.parent.value[thiskey];
    delete this.parent.vref[thiskey];
    delete this.tree._sources[source.filename];
    // delete this.tree;
    // delete this.vref;
    return this.recurseVrefDelete(deletedVref);
  }
...
Vertex.prototype.recurseSourceDelete = function(vref, tref, source) {
  ...
  if (vertex.sources.length == 1 && vertex.sources[0].filename == source.filename) {
    ...
    console.log(vertex.sources[0].filename);
    console.log('WRONG', vertex.parent.vref);
  } else {
    // remove the source
