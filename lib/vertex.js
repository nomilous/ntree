...
Vertex.prototype.DuplicateKeyError = function() {
  return new Errors.DuplicateKeyError(this.route, tools.listFileSources(this));
}
...
Vertex.prototype.loadSource = function(source, loading, change, overlap) {
  ...
  if (typeof value !== 'object') { // TODO: native types, support array
    if (this.sources.length > 1) {
      process.nextTick(function() { // so that error still as view of both
        _this.sources.pop();
      });
      throw this.DuplicateKeyError();
    }
    this.value = value;
  } else {
...
Vertex.prototype.loadKey = function(loading, source, vref, tref, key, value, change, parentOverlap) {
  ...
  if (typeof value !== 'object') {
    if (vertex.sources.length > 1) {
      ...
      throw vertex.DuplicateKeyError();
    }
    vertex.value = value;
    return;
  }
