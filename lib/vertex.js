...
function Vertex(tree, info) {
  ...
  debugs.new('name: \'%s\', route: %j, fullname: \'%s\'', info.name, info.route, info.fullname);
...
Vertex.prototype.init = Promise.promisify(function(callback) {
  ...
  debugs.init('at vertex name: \'%s\', route: %j', name, route);
  ...
  if (typeof this._info.stat == 'undefined') {
    ...
    debugs.init('load missing stat at vertex name: \'%s\'', name);

    fs.statAsync(this._info.fullname)
...
Vertex.prototype.loadDirectory = Promise.promisify(function(callback) {
  ...
  debugs.loadDirectory('at vertex name: \'%s\', directory: \'%s\'', name, fullname);
...
/**
 * ## .createVertexInfo(promise)
 *
 * Accepts the promise of a directory listing and returns the promise
 * of a list of corresponding vertex configs (_info)
 *
 * @api private
 * @params {Promise} list
 *
...
Vertex.prototype.createVertexInfo = function(list) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    Promise.map(list, function(item) {
      ...
      debug('found: \'%s\', at vertex name: \'%s\'', item, name);
      ...
      // full list become available, some with stat and some
...
Vertex.prototype.define = function(locus, object, value, route) {
  ...
  try {
    ...
    if (typeof object[locus] != 'undefined') {
      console.log('Already', object[locus]);
    }
    ...
    Object.defineProperty(object, locus, {
      ...
      // 
      // This then allows for overlapping non-leaf vertexes/routes
      //
      //   eg.
      //
