...
Vertex.prototype.getValue = function() {
  // TODO: if deleted
  var filtered, confineToSource = this.tree._confine;
  if (confineToSource) {
    if (typeof this.value !== 'object') return this.value;
    filtered = {};
    for (var key in this.value) {
      if (this.vref[key].__.hasSource(confineToSource)) {
...
Vertex.prototype.selectSource = function() {
  ...
  var i;
  if (this.tree._opts.source.select === 'deep') {
...
Vertex.prototype.onChanged = function(keyChanges) {
  var parentOverlap = false; // to not add to previousKeys in loadKey()
  var change, source, key, value, deletedVertex;

  var changes = {};

  if (keyChanges.added) {
    // select source
    // - for file to syncOut to
    // - for source to assign to the new vertices
    source = this.selectSource();
    change = changes[source.filePath] = changes[source.filePath] || tools.createChange(this.tree._opts, source);
    for (var i = 0; i < keyChanges.added.length; i++) {
      key = keyChanges.added[i];
      value = this.value[key]; // as just detected added
      delete this.value[key];
      this.loadKey(false, source, this.vref, this.value, key, value, change, parentOverlap, 'add');
    }
    this.syncOut(source/*, change */);
  }

  if (keyChanges.removed) {
    for (var i = 0; i < keyChanges.removed.length; i++) {
      key = keyChanges.removed[i];
      deletedVertex = this.vref[key].__;
      if (deletedVertex.sources.length == 1) {
        source = deletedVertex.sources[0];
        change = changes[source.filePath] = changes[source.filePath] || tools.createChange(this.tree._opts, source);
        deletedVertex.delete(source, change);
        this.syncOut(source/*, change */);
      } else {
        console.log('TODO: remove', deletedVertex);
      }
    }
  }
...
Vertex.prototype.syncOut = function(source /*, change */) {
  if (!this.tree._opts.syncOut) return;
  if (source.type !== SourceType.FILE) return;
  try {
    this.tree._skipInSync = source; // So that the changed file does not sync back in
    this.tree._confine = source; // So that the write to source includes only
                                 // the part of the tree it defined
