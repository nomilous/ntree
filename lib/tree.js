...
Tree.prototype._assemble = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    tree.on('$unload', tree._detatchSource.bind(tree));
    ...
    watcher.on('add', handlers.add = function(filename, stat) {
    ...
    watcher.on('addDir', handlers.addDir = function(filename, stat) {
    ...
    watcher.on('unlink', handlers.unlink = function(filename, stat) {
      stat = fakeStat(false, stat);
      emitter.emit('$unload', {
        filename: filename,
        loading: loading,
    ...
    watcher.on('unlinkDir', handlers.unlinkDir = function(filename, stat) {
      stat = fakeStat(true, stat);
      emitter.emit('$unload', {
        filename: filename,
        loading: loading,
    ...
    watcher.once('ready', function() {
      ...
      if (!tree._opts.syncIn) {
        for (var event in handlers) {
          watcher.removeListener(event, handlers[event]);
        }
        watcher.close();
      }
      tree._activate();
