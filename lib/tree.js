...
function Tree(opts) {
  var source, watcher, agent, doc, patch, keep;
  ...
  source.select = typeof source.select !== 'undefined' ? source.select : 'deep';
  ...
  // when writing it is usefull to be able to write only the part
  // of the tree defined in a particular source
  var confined;
  ...
  // when changes occur in tree and source files are updated (syncOut)
  // then if syncIn is also enabled the result file change will cause a
  // sync back in, use this to ignore the second sync
  var skipInSync = {};
  var testSkip = function(source) {
    var filePath = source.filePath;
    if (typeof skipInSync[filePath] === 'undefined') return false;
    if (skipInSync[filePath] == 1) {
      delete skipInSync[filePath];
      return true;
    }
    skipInSync[filename]--;
    return true;
  }
  Object.defineProperty(this, '_skipInSync', {
    enumerable: false,
    configurable: false,
    set: function(source) {
      var filePath = source.filePath;
      if (skipInSync[filePath]) {
        skipInSync[filePath]++;
        return;
      }
      skipInSync[filePath] = 1;
    },
    get: function() {
      return testSkip;
    }
...
Tree.prototype._updateSource = function(filename) {
  ...
  var vertex, change;
  if (this._skipInSync(source)) {
    debug('_updateSource skipping %s', filePath);
    return;
  }
  if (!source) {
